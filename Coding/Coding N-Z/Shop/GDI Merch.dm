...
obj/NPC
	GDI_Merchant
		var/tmp/list
			buylistweapon=newlist(/obj/Pickup/Guns/GDI_Carbine, /obj/Pickup/Guns/gdi_rifle,/obj/Pickup/Guns/Vulture_Assault_Shotgun, /obj/Pickup/Guns/Flacon, /obj/Pickup/Guns/GDI_Handgun, /obj/Pickup/Guns/MagnumG, /obj/Pickup/Guns/M60, /obj/Pickup/Guns/rap, /obj/Pickup/Guns/FiftyCaliber_G)
			buylistitem=newlist(/obj/Pickup/Items/QS/Radio_G,/obj/Pickup/Items/Launcher_Parts,/obj/Pickup/Items/Hammer,/obj/Pickup/Items/Turret,/obj/Pickup/Items/Ammo_Light_Handgun,/obj/Pickup/Items/Ammo_Heavy_Handgun,/obj/Pickup/Items/Ammo_Shotgun,/obj/Pickup/Items/Ammo_Light_Machinegun,/obj/Pickup/Items/Ammo_Heavy_Machinegun,/obj/Pickup/Items/Ammo_Sniper,/obj/Pickup/Items/Ammo_Sniper2,/obj/Pickup/Items/Ammo_Acid,/obj/Pickup/Items/Ammo_Flame,/obj/Pickup/Items/Ammo_Freeze,/obj/Pickup/Items/Ammo_Explosive,/obj/Pickup/Items/Spray,/obj/Pickup/Items/TCure,/obj/Pickup/Items/C4,/obj/Pickup/Items/TripWire)
		name="GDI Merchant"
		icon='Merchant.dmi'
		icon_state="normal_G"
		density=1
		verb/Shop()
			...
			M.shopping = 1
			switch(tm_option(M,null,"Got a good collection on sale soldier.","Merchant","300x340",list("Buy","Upgrade","Leave"),"select list"))
				if("1")
					buy
					if(!src||!M)return
					if(!(src in oview(1))||GameOn!=1){M.shopping=0;return}
					switch(tm_option(M,null,"Got a good collection on sale soldier.","Merchant","300x340",list("Weapons","Items","Cancel"),"select list"))
						if("1")
							if(!src||!M)return
							...
							for(var/i=1,i<=length(List),i++)
								...
								Listing+={"
									...
									"}
							var/confirm = tm_option(M,null,"Whata you purchasin?","Merchant","460x400",Listing,"select custom list")
							if(confirm)
								if(confirm<=length(List))
									if(!src||!M)return
									var/obj/Pickup/Guns/G=List[confirm]
									if(G.cost>M.cash){M<<"Not enough cash... soldier....";goto buy}
						...
						if("2")
							...
							for(var/i=1,i<=length(List),i++)
								...
								Listing+={"
									...
									<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>(-) Cost- [G.cost]</td></tr>
									"}
							var/confirm = tm_option(M,null,"Whata you purchasin?","Merchant","410x400",Listing,"select custom list")
							if(confirm)
								if(confirm<=length(List))
									if(!src||!M)return
					...
					switch(tm_option(M,null,"What would you like to upgrade?","Merchant","300x340",list("Weapon","Other","Cancel"),"select list"))
						...
						if("2")
							...
							switch(tm_option(M,null,"What will you upgrade?","Merchant","410x400",L,"select custom list"))
								if(1)
									if(!src||!M)return
									if(8000>M.cash){M<<"Not enough cash... soldier....";goto upgrade}
