...
obj/NPC
	Merchant
		var/tmp/list
			buylistweapon=newlist(/obj/Pickup/Guns/Glock_18,/obj/Pickup/Guns/Magnum,/obj/Pickup/Guns/Spaz_12,/obj/Pickup/Guns/Pump_Shotgun,/obj/Pickup/Guns/Sawnoff,/obj/Pickup/Guns/MAC11_SubMachine_Gun,/obj/Pickup/Guns/MP5,/obj/Pickup/Guns/UMP45,/obj/Pickup/Guns/Uzi,/obj/Pickup/Guns/Assualt_Rifle,/obj/Pickup/Guns/M4_Carbine,/obj/Pickup/Guns/M16,/obj/Pickup/Guns/AK_47,/obj/Pickup/Guns/Ultimax_100_MK4_LMG,/obj/Pickup/Guns/AUG,/obj/Pickup/Guns/G3SG1,/obj/Pickup/Guns/M249,/obj/Pickup/Guns/MosinNagant,/obj/Pickup/Guns/FiftyCaliber,/obj/Pickup/Guns/Grenade_Launcher,/obj/Pickup/Guns/Rocket_Launcher)
			buylistitem=newlist(/obj/Pickup/Items/Launcher_Parts,/obj/Pickup/Items/Hammer,/obj/Pickup/Items/Turret,/obj/Pickup/Items/Ammo_Light_Handgun,/obj/Pickup/Items/Ammo_Heavy_Handgun,/obj/Pickup/Items/Ammo_Shotgun,/obj/Pickup/Items/Ammo_Light_Machinegun,/obj/Pickup/Items/Ammo_Heavy_Machinegun,/obj/Pickup/Items/Ammo_Sniper,/obj/Pickup/Items/Ammo_Sniper2,/obj/Pickup/Items/Ammo_Acid,/obj/Pickup/Items/Ammo_Flame,/obj/Pickup/Items/Ammo_Freeze,/obj/Pickup/Items/Ammo_Explosive,/obj/Pickup/Items/Spray,/obj/Pickup/Items/TCure,/obj/Pickup/Items/C4,/obj/Pickup/Items/TripWire)
		name="Merchant"
		icon='Merchant.dmi'
		icon_state="normal"
		density=1
		verb/Shop()
			...
			M.shopping = 1
			switch(tm_option(M,null,"Got a good collection on sale stranger.","Merchant","300x340",list("Buy","Upgrade","Leave"),"select list"))
				if("1")
					buy
					if(!src||!M)return
					if(!(src in oview(1))||GameOn!=1){M.shopping=0;return}
					switch(tm_option(M,null,"Got a good collection on sale stranger.","Merchant","300x340",list("Weapons","Items","Cancel"),"select list"))
						if("1")
							if(!src||!M)return
							...
							for(var/i=1,i<=length(List),i++)
								...
								Listing+={"
									...
									"}
							var/confirm = tm_option(M,null,"Whata you buyin?","Merchant","460x400",Listing,"select custom list")
							if(confirm)
						...
						if("2")
							...
							for(var/i=1,i<=length(List),i++)
								...
								Listing+={"
									...
									<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>(-) Cost- [G.cost]</td></tr>
									"}
							var/confirm = tm_option(M,null,"Whata you buyin?","Merchant","410x400",Listing,"select custom list")
							if(confirm)
								if(confirm<=length(List))
									if(!src||!M)return
				...
				if("2")
					...
					if(!src||!M)return
					if(!(src in oview(1))||GameOn!=1){M.shopping=0;return}
					switch(tm_option(M,null,"What would you like to upgrade?","Merchant","300x340",list("Weapon","Other","Cancel"),"select list"))
						if("1")
							if(!src||!M)return
							...
							for(var/i=1,i<=length(List),i++)
								...
								if(G.can_upgrade_fr && G.can_upgrade_ac)
									...
									Listing+={"
										...
										<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.reloadtime_level]) <b>Reload Speed</b>- [G.reload_time]</td></tr>
										"}
							var/confirm = tm_option(M,null,"Upgrade which weapon?","Merchant","350x340",Listing,"select custom list")
							if(confirm)
								if(confirm<=length(List))
									if(!src||!M)return
									var/obj/Pickup/Guns/G=List[confirm]
...
obj/Other
	...
	Chest
		...
		verb/Open()
			...
			set category = null
			var/mob/player/client/M=usr
			if(!(src in oview(1))||M.shopping||M.tm_inmenu)return
			switch(tm_option(M,null,"What would you like to do?","Chest","300x340",list("Take","Store","Cancel"),"select list"))
				if("1")
					again
					if(!src||!M)return
					...
					if(length(M.chest))
						var/list/List=new/list()
						var/list/Listing=new/list()
						for(var/obj/Pickup/Guns/P in M.chest)List+=P
						for(var/i=1,i<=length(List),i++)
							var/obj/Pickup/Guns/G=List[i]
							Listing+={"
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td align=left><font size=3>[i] <b>- [G.name]</b></td><td align=right><font size=3>\[<a href=?src=\ref[M];action=i;i=[length(Listing)+1]>Take</a>\]</td></tr>
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.clip_level]) <b>Capacity</b>- [G.mclip]</td></tr>
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.firerate_level]) <b>Firing Speed</b>- [G.firerate]</td></tr>
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.gunpower_level]) <b>FirePower</b>- [G.fire_power]</td></tr>
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.reloadtime_level]) <b>Reload Speed</b>- [G.reload_time]</td></tr>
								"}
						var/confirm = tm_option(M,null,"Take what?","Chest [M.chest.len]/[M.chest_capacity]","320x260",Listing,"select custom list")
						if(confirm)
							if(confirm<=length(List))
								if(!src||!M)return
								var/obj/Pickup/Guns/G=List[confirm]
								if(G&&length(M.chest)&&M.chest.Find(G))
									if(M.items>=M.maxitems){M<<"Cant carry anymore items";return}
									M.contents+=G
									M.chest-=G
									M.update_items()
									M.save_game()
								goto again
					else {M<<"Chest: [length(M.chest)]/[M.chest_capacity]";return}
				if("2")
					again
					if(!src||!M)return
					if(!(src in oview(1)))return
					if(length(M.contents)&&length(M.chest)<M.chest_capacity)
						var/list/List=new/list()
						var/list/Listing=new/list()
						for(var/obj/Pickup/Guns/P in M.contents)List+=P
						for(var/i=1,i<=length(List),i++)
							var/obj/Pickup/Guns/G=List[i]
							Listing+={"
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td align=left><font size=3>[i] <b>- [G.name]</b></td><td align=right><font size=3>\[<a href=?src=\ref[M];action=i;i=[length(Listing)+1]>Store</a>\]</td></tr>
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.clip_level]) <b>Capacity</b>- [G.mclip]</td></tr>
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.firerate_level]) <b>Firing Speed</b>- [G.firerate]</td></tr>
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.gunpower_level]) <b>FirePower</b>- [G.fire_power]</td></tr>
								<tr align=left bgcolor=[SUBBGCOLORWINDOW]><td colspan=2><font size=3>Lv.([G.reloadtime_level]) <b>Reload Speed</b>- [G.reload_time]</td></tr>
								"}
						var/confirm = tm_option(M,null,"Store what?","Chest [length(M.chest)]/[M.chest_capacity]","320x340",Listing,"select custom list")
						if(confirm)
							if(confirm<=length(List))
								if(!src||!M)return
								var/obj/Pickup/Guns/G=List[confirm]
								if(G&&length(M.contents)&&M.contents.Find(G))
									if(M.weapon&&M.weapon==G)
										if(M.weapon2)
											G.identify_ammo(M,1)
											G.suffix=null
											M.overlays-=G.overlay_add
											M.weapon=null
											for(var/obj/Pickup/Guns/G22 in M.contents)
												if(G22==M.weapon2){M.weapon2=null;M.weapon=G22;break}
